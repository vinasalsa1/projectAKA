...
if st.button("Hitung Variabel"):
    ...
    if result is not None:
        ...
        if var == "n":
            st.latex(f"n = \\frac{{P \\times V}}{{R \\times T}} = \\frac{{{P:.4f} \\times {V:.4f}}}{{{R:.6f} \\times {T:.2f}}} = {result:.4f} \\text{{ mol}}")
        elif var == "V":
            st.latex(f"V = \\frac{{n \\times R \\times T}}{{P}} = \\frac{{{n:.4f} \\times {R:.6f} \\times {T:.2f}}}{{{P:.4f}}} = {result:.4f} \\text{{ {unit}}}")
        elif var == "P":
            st.latex(f"P = \\frac{{n \\times R \\times T}}{{V}} = \\frac{{{n:.4f} \\times {R:.6f} \\times {T:.2f}}}{{{V:.4f}}} = {result:.4f} \\text{{ {unit}}}")
        elif var == "T":
            st.latex(f"T = \\frac{{P \\times V}}{{n \\times R}} = \\frac{{{P:.4f} \\times {V:.4f}}}{{{n:.4f} \\times {R:.6f}}} = {result:.4f} \\text{{ K}}")
    else:
